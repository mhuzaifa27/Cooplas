...
import kotlinx.android.synthetic.main.likeitem.view.*
...
class LikesAdapter(
    var context: Context,
    var likes_list: ArrayList<LikesData>
) : RecyclerView.Adapter<LikesAdapter.LikeView>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LikeView {
        return LikeView(LayoutInflater.from(context).inflate(R.layout.likeitem, parent, false))
    }

    override fun onBindViewHolder(holder: LikeView, position: Int) {
        val item = likes_list[position]
        Glide.with(context).load(item.user.profile_pic).into(holder.iv_profile)
        holder.name.text = "${item.user.first_name} ${item.user.last_name}"
        when (item.user.following) {
            0 -> {
        ...
        holder.follow.setOnClickListener {
            holder.follow.visibility = View.GONE
            holder.unfollow.visibility = View.VISIBLE
            follow_user(item.user_id)
        }
        holder.unfollow.setOnClickListener {
                holder.follow.visibility = View.VISIBLE
                holder.unfollow.visibility = View.GONE
                unfollow_user(item.user_id)
        }
    }
    ...
    class LikeView(view: View) : RecyclerView.ViewHolder(view) {
