package com.example.cooplas.Firebase.Services;
...
public class UserService implements ChildEventListener, ValueEventListener {
    ...
    public UserService(){
            mLanguagePreference = FirebaseDatabase.getInstance().getReference("allUsers").child("Customer");
    ...
    public User getUserIdById(String id){
        ...
        for (DataSnapshot snapshot : mSnapshots) {
            try {
                User User = snapshot.getValue(User.class);
                if (User != null && User.id != null && User.id.equalsIgnoreCase(id)) {
                    return snapshot.getValue(User.class);
                }
            } catch (Exception e) {
                e.printStackTrace();
